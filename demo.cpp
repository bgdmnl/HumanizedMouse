...
int main() {
    ...
    // Longer duration for human-like slow movement
    std::uniform_real_distribution<double> speed_dist(1.5, 3.2);
    std::uniform_real_distribution<double> curve_dist(0.2, 0.5);

    HumanMouseMover mover;

    std::cout << "Demo: Mouse will move to random locations on screen with longer transition time." << std::endl;
    ...
    while (true) {
        ...
        opts.duration_sec = speed_dist(rng);   // random long duration
